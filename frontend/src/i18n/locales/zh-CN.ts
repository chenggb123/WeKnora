export default {
  ...
  settings: {
    ...
    parser: {
      ...
      tierLabel: '解析档位（MinerU 4.0+）',
      tierFlash: 'flash — 原生文本解析，最快',
      tierBasic: 'basic — 小模型（OCR/公式/表格）',
      tierStandard: 'standard — 默认质量（推荐）',
      tierAdvanced: 'advanced — 最高质量，耗时更长',
      tierHint: 'MinerU 4.0 起用解析档位替代 Backend。端点为 4.0 服务时，下方的 Backend、vLLM 地址与识别选项会被忽略；旧版（≤3.x）服务仍按这些参数解析。standard / advanced 档位需要服务端具备 VLM（GPU）能力，CPU 环境请使用 flash 或 basic。',
