// Package textmetrics provides language-aware token count approximation shared
// by the chunker (which sizes chunks against an embedding model's window) and
// the embedding clients (which report over-window inputs in the unit the
// provider actually enforces).
//
// It deliberately has no imports from the rest of the module: the embedding
// package needs these numbers, and the chunker package it would otherwise reuse
// already sits behind a dependency cycle (chunker → docparser → searchutil →
// types/interfaces → models/embedding).
//
